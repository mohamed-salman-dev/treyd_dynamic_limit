...
def flat_history(months: list[str], amount: float, currency: str = "GBP") -> list[MonthlyAmount]:
    return [MonthlyAmount(month=m, amount=amount, currency=currency) for m in months]
...
def seasonal_history(start: str, count: int, base: float, currency: str = "GBP") -> list[MonthlyAmount]:
    ...
    for ym in month_range(start, count):
        cal = int(ym[5:])
        out.append(MonthlyAmount(month=ym, amount=base * SEASONAL_PATTERN[cal], currency=currency))
    return out
