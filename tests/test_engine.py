...
def test_base_months_pre_launch_is_3():
    # onboarding: 0 routing days, full history credit (capped at 6) → ET 6 → 3.0
...
def test_base_months_four_earned_by_tenure():
    # 210 routing days (=7 months) + 6 history credit → ET 13 → 4.0, no override needed
    base, et, used = _base_months(routing_days=210, verified_api_months=24, override=None)
    assert et == 13.0
    assert base == 4.0
